  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-100 overflow-hidden">
      <div className="p-5 border-b border-slate-100 flex items-center justify-between">
        ...
        <button onClick={onNewRule} className="px-4 py-2 rounded-xl text-xs font-semibold text-white cursor-pointer whitespace-nowrap" style={{ background: '#0D1F4A' }}>
      ...
      <div className="divide-y divide-slate-100">
        {automations.map((rule) => (
          <div key={rule.id} className="p-4 flex items-start gap-4 hover:bg-slate-50/50 transition-colors">
            <div className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0" style={{ background: rule.status ? '#0D1F4A15' : '#F1F5F9' }}>
              <i className="ri-robot-2-line text-sm" style={{ color: rule.status ? '#0D1F4A' : '#94A3B8' }} />
            </div>
