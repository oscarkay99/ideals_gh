  ...
  return (
    <AdminLayout title="SMS Campaigns" subtitle="Bulk Messaging · Payment Reminders · Delivery Updates">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4 mb-5">
        {[
          { label: 'Total Sent', value: smsStats.totalSent.toLocaleString(), icon: 'ri-send-plane-line', color: '#1E5FBE' },
          ...
          { label: 'Credits Left', value: smsStats.creditsRemaining.toLocaleString(), icon: 'ri-coin-line', color: '#0A1F4A' },
        ].map((s) => (
          ...
          <div className="divide-y divide-slate-100">
            {smsCampaigns.map((campaign) => (
              <div key={campaign.id} className="p-4 flex items-start gap-4 hover:bg-slate-50/50 transition-colors">
                <div className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0" style={{ background: campaign.type === 'Promotional' ? '#F5A62315' : '#1E5FBE15' }}>
                  <i className={`${campaign.type === 'Promotional' ? 'ri-megaphone-line' : 'ri-mail-send-line'} text-lg`} style={{ color: campaign.type === 'Promotional' ? '#F5A623' : '#1E5FBE' }} />
                </div>
          ...
          {smsSegments.map((segment) => (
            <div key={segment.id} className="bg-white rounded-2xl p-4 border border-slate-100">
              <div className="flex items-center justify-between mb-3">
                ...
                <span className="text-lg font-bold" style={{ color: '#1E5FBE' }}>{segment.count.toLocaleString()}</span>
              </div>
              <p className="text-xs text-slate-500 mb-4">{segment.description}</p>
              <div className="flex items-center gap-2">
                <div className="flex-1 h-1.5 rounded-full bg-slate-100 overflow-hidden">
                  <div className="h-full rounded-full" style={{ width: `${(segment.count / 2840) * 100}%`, background: '#1E5FBE' }} />
                </div>
              ...
              <button
                ...
                style={{ borderColor: '#1E5FBE', color: '#1E5FBE' }}
              >
                Send to Segment
              </button>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          <div className="bg-white rounded-2xl p-5 border border-slate-100">
            ...
            <div className="space-y-4">
              {[
                { label: 'Sent', value: smsStats.totalSent, max: smsStats.totalSent, color: '#1E5FBE' },
                ...
                { label: 'Converted to Leads', value: smsStats.totalLeads, max: smsStats.totalSent, color: '#0A1F4A' },
              ].map((item) => (
                <div key={item.label}>
          ...
          <div className="bg-white rounded-2xl p-5 border border-slate-100">
            ...
            <div className="flex items-center justify-center py-8">
              <div className="relative w-40 h-40">
                <svg viewBox="0 0 100 100" className="w-full h-full -rotate-90">
                  <circle cx="50" cy="50" r="40" fill="none" stroke="#EEF4FF" strokeWidth="12" />
                  <circle
                    cx="50" cy="50" r="40" fill="none" stroke="#1E5FBE"
                    strokeWidth="12"
            ...
            <div className="flex items-center justify-center gap-6 text-xs">
              <div className="flex items-center gap-2">
                <span className="w-3 h-3 rounded-full" style={{ background: '#1E5FBE' }} />
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setShowCompose(false)} className="w-8 h-8 rounded-lg flex items-center justify-center hover:bg-slate-100 cursor-pointer">
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-3">
                <button onClick={() => setShowCompose(false)} className="flex-1 py-3 rounded-xl text-sm font-semibold text-slate-500 border border-slate-200 cursor-pointer whitespace-nowrap">
                  Save Draft
                </button>
                <button onClick={() => setShowCompose(false)} className="flex-1 py-3 rounded-xl text-sm font-semibold text-white cursor-pointer whitespace-nowrap" style={{ background: '#1E5FBE' }}>
                  Send Campaign
                </button>
              </div>
            </div>
